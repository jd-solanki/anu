...
export const ADrawer = defineComponent({
  ...
  setup(props, { slots, emit, attrs }) {
    ...
    return () => <Teleport to="body">
      <Transition name="bg">
        <div v-show={props.modelValue} class={[`a-drawer-wrapper a-drawer-wrapper-anchor-${props.anchor} grid fixed uno-layer-base-inset-0 bg-[hsla(var(--a-overlay-color),var(--a-overlay-opacity))]`]}>
          <Transition name={`slide-${props.anchor}`}>
            <ACard {...attrs} v-show={props.modelValue} class={['a-drawer backface-hidden transform translate-z-0 rounded-none max-w-[calc(100vw-2rem)]', '']} ref={refCard} {...props}>{{ ...slots }}</ACard>
          </Transition>
        </div>
      </Transition>
    </Teleport>
